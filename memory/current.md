...
- **N4 (сверка 2026-09-16/17): два опыта проверены, полная приёмка с тремя P2 не закрыта** —
  ТЗ `memory/req-object-resonators-n4-2026-09-16.md`; лог `memory/log/2026-09-16-object-resonators-n4.md`.
  Движок `ca_object_resonators` (каждая связная фигура — банк резонаторов своего лапласиана,
  удары от изменений клеток внутри её круга/своих клеток; цветные фигуры, круги, центры в стенде).
  Каталог `lab_catalog/object_resonators_n4_2026_09_16/` (2 записи, гипотезы в Notes), вход
  `run_object_resonators_n4.bat`; заметки:
  `python -m casynth_lab.catalog notes lab_catalog/object_resonators_n4_2026_09_16`.
  Старый синт цел: Laplacian golden master байт-в-байт, UI `gol_synth` пиксель-в-пиксель, все
  прежние гейты; записи N3 воспроизводятся. Измерения == preflight до 0.02 dB (N4.1 Б −40.1 dBFS,
  N4.2 −35.6/−35.2; Disk−Own −11.1 dB отн. Own), приёмник Own 0 / Disk 4 на поколение, glider —
  один id и два чередующихся спектра, p99 обеих сторон ≈ 1–1.5 ms. Отчёт
  `demos/results/object_resonators_n4/report.md`. **Открыто (`questions.md`):** ×0.5 при клипе в
  стресс-пробах (gain 0.04), hard drops при замене всего поля каждый блок, анализ фигур > ~300
  клеток дороже бюджета блока (в сценах ≤ 17 клеток), N4.1 А (N3) тише Б на 2.25 dB.
  **Ревью Researcher получено** (`memory/research/object-resonators-n4-review-2026-09-16.md`): опыты
  соответствуют гипотезам; три P2 к реализации (возвращающиеся моды наследуют хвосты; обрыв хвоста
  при переполнении пула затухающих; пик 2.565 пробы хвостов не в сводке отчёта) — **ждут
  исправления**. Добавлена ручка «Radius x» (множитель радиуса Disk, дефолт 1.0), каталог
  пересобран с тем же звуком.
  Researcher повторно проверил 28 тестов N4 после Radius x и точный пересчёт обеих новых записей.
  **Следующее задание пользователя, 2026-09-17:** сравнение старого Laplace с Objects и все
  настройки Laplace в Objects. ТЗ `memory/req-objects-laplace-comparison-2026-09-17.md`:
  три пары, общий спектральный режим/настройки, Radius x, подготовленные клетки и гипотезы.
  Новый режим и новый каталог ещё не реализованы; слухового результата N4 пока нет.
