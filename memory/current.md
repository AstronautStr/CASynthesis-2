...
| Файл | Что это |
|---|---|
| `gol_synth.py` | **Активный прототип**: event-loop, layout, MIDI, VCA; ХОСТИТ экземпляр `SoundEngine` из `casynth_engines` (вкладки — движки, умеющие играть ноту). **2026-09-22** (лог `memory/log/2026-09-22-instrument-panel-and-save.md`): открывается на `DEFAULT_ENGINE = laplace_unified` и на `START_SPECTRUM` (n 20, spread 1, alpha 0, shape 1, harm 1, full on, dyn 1 — накладывается на дефолты реестра у всех движков с полным лапласовым спектром; сами дефолты реестра не трогаются: они — нейтральная точка чужих гейтов и таблиц ТЗ); у каждого слайдера поля Min/Max (значение переехало внутрь трека), панель перестраивается, когда слайдер меняет набор неактивных рядов; кнопка **Save** превращает сыгранное в запись каталога — в ОТДЕЛЬНОМ потоке (лог `memory/log/2026-09-22-save-hang.md`): клик снимает копию журнала сессии и отдаёт её воркеру, кадровый цикл не блокируется (было: 28 с без единого кадра на минуту игры), тулбар и консоль показывают прогресс рендера, выход `join`-ит воркера; запись больше не обрезается кольцом рекордера на 30 с; журнал сессии пишется всегда (аудио-чанки — только при `CASYNTH_RECORD=1`, `CASYNTH_NO_RECORD=1` выключает журнал), тестовый хук `CASYNTH_PANEL_LOG` |
...
| `casynth_lab/offline_record.py` | **Сцена → запись каталога оффлайн** (2026-09-22): `DemoRunner` + `Recorder` + `Catalog.save` — один путь для `demos/build_*` и для кнопки Save прототипа; кольцо `Recorder` сайзится под длину рендера (у живого стенда оно скользящее, 30 с — рендер длиннее возвращался обрезанным); в `SOUND_EXCLUDE` (отпечаток звука не трогает) |
...
**Тиры (2026-09-23, лог `memory/log/2026-09-23-gate-order.md`):** `python check.py` после
каждого изменения = быстрый набор + гейты, которые задевают файлы, изменённые относительно
HEAD (карта «файл → гейты» в `check.py` + поиск имени файла в текстах гейтов; незнакомый файл
запускает всё); `python check.py --all` перед каждым коммитом = все гейты. `--dry-run`
показывает выбор, `--files=a,b` / `--since=REV` переопределяют «изменённое».
**Как гоняется полный прогон (2026-09-22, лог `memory/log/2026-09-22-instrument-panel-and-save.md`):**
две волны — сначала гейты, которые МЕРЯЮТ ВРЕМЯ (по одному, на неразогретой машине; пауза
`COOLDOWN_S = 6` — перед первым и после гейта, который бежал дольше `COOLDOWN_AFTER_S = 2` с,
после секундных не спим — 2026-09-23), потом всё остальное пулом на 6 процессов с
`CASYNTH_SKIP_TIMING=1`, **самые долгие гейты первыми** (порядок — по временам прошлого прогона
из `artifacts/gate_times.json`, до него — по оценкам `EXPECT_S`; FIFO по списку ждал в конце
один отставший гейт) и с `OPENBLAS_NUM_THREADS=2` у процессов волны (OpenBLAS резервирует буферы
на все 20 ядер в каждом из ~15 процессов волны — это и было `OpenBLAS error: Memory allocation
still failed` в S7 / S/N / articulation на 16 ГБ; побайтовые эталоны при 1 и 2 потоках те же).
**Замер 2026-09-23: 304 с → 230 с**, красные только известные (лог `memory/log/2026-09-23-gate-order.md`).
...
**Аудит покрытия 2026-09-23** (`memory/log/2026-09-23-test-coverage-audit.md`): звуковой набор
исполняется тестами на 93–99 % строк и держится байтовыми/скалярными эталонами; дыры — ветви
кражи/вытеснения хвостов `SlotPool` (не исполняются ничем), живой тракт `gol_synth` (live↔offline
не сверяется), стартовая позиция (только красный `ui frame`), пины записей по локальным wav.
Параллельная волна на 16 ГБ может ронять гейты по памяти (S7, articulation) — красное по
`Out of memory`/`OpenBLAS` перепроверять поодиночке.
**Закрыто в тот же день (§8 лога):** гейт **2b** `tests/golden/tail_pool_master.py` — пул хвостов в
перегрузе (вытеснение + кража) побайтно против эталона, снятого на `f3e8518` до векторизации (в
`--fast`); гейт **3g** `tests/ui_fidelity_probe.py` — живая сессия прототипа == офлайн-рендер её сцены
байт-в-байт (поколения на аудио-часах, VCA, нота, хостинг). Попутно `scene_from_session` ставит ручные
правки поля на семпл применения из `field_applied` (было — из кадрового лога, ±1 блок; фаза мод
уезжала навсегда) — это же чинит записи Save. **Открыто:** стенд считает возбуждение `dyn` на ручной
правке, прототип — нет (`questions.md` 2026-09-23); при `dyn` > 0 запись Save звучит не так, как
инструмент, проба пока рендерит по семантике прототипа.
...
- **Аудит инфраструктуры (Developer, 2026-09-22): `memory/audit-infra-2026-09-22.md` — P1 (порядок волны,
  cooldown по делу) СДЕЛАН 2026-09-23 (304 → 230 с), плюс лимит потоков OpenBLAS в волне против падений по
  памяти; P3 (тиры: дефолт по изменённым файлам, `--all` перед коммитом) СДЕЛАН 2026-09-23;
  ЖДЁТ РЕШЕНИЙ D3–D4 и «известные красные» (P1.3).**
  Полный `check.py` сегодня 304 с (970 с гейт-времени): 48 с чистого cooldown, ~50 с на FIFO-порядке волны 2,
  231 с — real-time пробы прототипа (его логика — замыкания в `main()`, тестируется только целиком), две
  известные красные (`ui frame`, `live budget: Ev+FM`) держат `RESULT: FAIL`. Движки общие на 100 %,
  но хост вокруг них написан дважды (`gol_synth._render_loop` vs `DemoRunner`/`LiveEngine`; расхождения
  §5.2 отчёта), равенство двух хостов гейтится только для `laplacian`, мост Save молча теряет драги
  ручек/GEN/BPM. Предложено: P1 три правки `check.py` (≈ −80 с), P2 пять правок тестов, P3 тиры +
  `--changed`, P4 прототип как второй UI над одним хостом.
- **Аудит производительности всего аудиотракта (Developer, 2026-09-22, только измерения, код не
  тронут) — `memory/log/2026-09-22-perf-audit.md`, ЖДЁТ ВЫБОРА.** Итог: Events+FM за бюджетом
  (8.5 мс холодная, 12.3 прогретая; 75 % — 3.2 млн `sin`, побитовых резервов ~1 мс, дальше — решение о
  законе/точности); драг ЛЮБОЙ спектральной ручки — самая дорогая операция (`SlotPool._acquire_tail`
  линейный скан 1920 хвостов на моду при темпе игрока, UI-`analyse` каждый кадр 5.5–23 мс,
  `_retune_all` = `eigh` × фигур на кадр, пул таблиц Events+Saw/Square 16 мс/блок); граница
  Events 10–18 мс; Python-циклы на потоке рендера платят вдвое из-за GIL. План A (побитово:
  кэш `eigh`, стек хвостов, `_release_quiet` в numba, слабы legacy, лимит таблиц за блок) /
  B (решения: oversampling/float32 sin/хвосты ФМ, ключ таблиц, WASAPI) / C (UI-кэш статики,
  дроссель анализа, look-ahead, snapshot стенда раз в кадр, GC).
  **Часть 1 сделана (лог `memory/log/2026-09-22-perf-fixes.md`, коммиты `80d3bf2`, `f3e8518`,
  `9849350`):** байтовый эталон Events-клеток (`tests/golden/events_master.py`, гейт `events
  master`); кэш `eigh`/`eigvalsh` по байтам матрицы в `casynth_core` (гейт-счётчик
  `test_knob_drag_work.py`); пул хвостов `SlotPool` векторно с тем же выбором слота
  (гейт-счётчик `test_slot_pool_work.py`). Живой драг `harm` на дефолтной клетке: 483 → 6
  андерранов, кадр снова 60 fps. **Пул таблиц волн 512 → 1024** (сессия игрока 22:46: поле не
  трогали, Events+Saw при `harm` < 1 звучит 630–1000 разных частот → пул тёр таблицы каждое
  поколение, 276 андерранов; гейт `test_wave_pool_holds_the_field.py`; эталон Events переснят —
  прежний пинил перегруз; по второй сессии 22:56 — **2048**, плотнее поле = до 1434 волн).
  **`SoundEngine.warm()`** — прогрев numba-ядер Events на UI-потоке при создании движка (клик
  `artic` стоил 159–189 мс на потоке рендера; гейт `test_events_kernels_warm.py`).
  **`gc.freeze()`** на первом кадре с устройством и движком (полная сборка 25–36 → 0.07 мс;
  гейт `test_gc_pause.py`). **Кадр в кэше** (`d2afe21`: текст по (шрифт, строка, цвет), поле /
  легенда / спектр / пианино / колонка — поверхности по ключу; пиксели те же; 4.3 → 1.4 мс;
  гейт-счётчик `test_frame_work.py`). **Поколения на аудио-часах** (`fe3dd77`, решение
  пользователя «да»): шаг автомата несёт кадр устройства, с которого звучит; цепочка штампов
  ровно по шагу с якорем дедлайн + look-ahead + `GEN_LEAD_BLOCKS`; `BlockRing.played/silence/
  position()`; журнал `rec['field_applied']`, сцена ставит шаги по нему; тайминговый гейт
  `test_generation_clock.py` (провалы рендера 70 мс: было до 75 мс мимо доли, стало в пределах
  блока). **Табличный синус в Events + FM** (2026-09-23, решение пользователя: 4096 точек +
  lerp, −122 дБ, max 1 LSB на 0.26 % значений, `CASYNTH_FM_SINE=libm` — точный; эталон Events
  переснят; блок 8.8 → 5.8 мс — внутри бюджета, гейт 0.6 ещё красный; oversampling 4 — только
  после спектральной сверки с 16×). Осталось побитового: `_release_quiet` в numba, слабы для
  legacy, квантование слайдера, Python-часть `_analyse`, та же заморозка GC в стенде. Не побитово (Researcher):
  ключ таблиц по числу гармоник (граница поколения на плотном поле ≈ 200 таблиц = 17–20 мс
  одним блоком; драг Events+Saw/Square 24 мс/блок), хвосты Events+FM. Известное старое:
  правки поля в сцене из сессии стоят по кадровому логу и расходятся с живой записью
  (первое ручное изменение) — территория инфра-аудита.
