...
- **Laplace FM (2026-09-20): реализовано, каталог собран и запинен (`e418449`), ждёт сверки Researcher и слуха** —
  ...
  **Бюджет блока на ЖИВОМ поле прототипа (2026-09-21/22, логи `2026-09-21-render-budget.md` и
  ...
  бюджете 7.98 — то есть на Saw и ФМ с этими настройками дропы возвращаются. Побитовый резерв
  выбран: дальше либо n поменьше, либо размен бит (float32-sin даёт ×6.6 по замеру, рекуррентные
  синусы, OVERSAMPLE 8→4) с ре-блессом golden master. Драг спектральной ручки в ФМ — 24 мс на блок.
