...
`python check.py` = 59 юнит-тестов + 42 + 9 + 6 + 6 + 6 + 15 тестов demo-стенда S1–S7 и демо S/N + 9 тестов N0 (гейт 1h) + 10 тестов N1 (гейт 1i) + golden-master аудио (байт-в-байт,
...
- **N0/N1 (2026-09-15): ревью Researcher завершено** —
  ...
  отчёт измерений `demos/results/network_n1/report.md`. **Ждёт:** ревью
  Researcher на соответствие ТЗ и слуховую оценку Пользователя в живом стенде (WAV-приёмки нет);
  заметки Пользователя вытаскивать `python -m casynth_lab.catalog notes lab_catalog/network_n1_2026_09_15`.
