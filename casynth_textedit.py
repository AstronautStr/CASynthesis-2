"""Text-field model shared by EVERY text input of both UIs: the bench's save
form (Title / Note) and listening Notes, and the prototype's Min / Max range
fields of a knob row (2026-09-22, when it moved out of casynth_lab so the
prototype could use it without importing the bench).  pygame-free: the view
supplies `measure` (text -> pixel width) and draws; this class owns text, caret,
selection, word rules, wrapping and pixel <-> caret mapping.  One model = one
behaviour for all fields (2026-09-16: the hand-rolled fields had no caret at all).
