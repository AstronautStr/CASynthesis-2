...
## 4h. `tests/test_n4_object_resonators.py` — N4: резонаторы фигур и круговые детекторы (27)
...
- `SnapshotTests` (2) — снапшот посреди хвоста, рампы r, рамп весов/панорамы, с ожидающим полем, через `snapshot.py`: 30 блоков побитово и равный `display()`; отказ при чужой модели, подправленном спектре, чужом ожидающем поле; `DemoRunner` обеих сцен с командами decay/scale: экспорт через файл, 60 блоков A/B/monitor побитово и равные `display`.
