...
## 4j. `tests/test_objects_birth_strength.py` — Objects: сила влияния места рождения (8)

ТЗ `memory/req-objects-event-source-modal-2026-09-17.md` раздел 5; fixtures `memory/research/objects-birth-strength-preflight-2026-09-18.json` (+ клетки M1 из preflight 2026-09-17); сцена `demos/obs_m2.json` (сборщик `demos/build_objects_birth_strength.py`); временные файлы в `artifacts/_obs_tests/`. Гейт 1p в `check.py`.

- `RegistryTests` (1) — ручка `birth_strength` 0…4 (float, default 1) сразу после `excitation`; отсутствие ключа = 1; модель `n4_v5` / STATE_VERSION 5, v4 в `COMPATIBLE_STATES`; отказ реестра / движка / снимка на −0.001, 4.001, NaN, ±inf, строке, bool (параметры движка при отказе не меняются); `inactive` при Uniform = `"2.50  Birth position only"`, при Birth position ручка активна; s = 0 не обходит условие сочетания; `display` отдаёт `birth_strength` и подсказку; все 12 сцен Objects (n4/ol/ora/oes/obs) грузятся, ключ 1.0 (у Б M2 — 4.0).
- `LawTests` (2) — `birth_strength_profile` против независимого preflight на всех фазах M1 и синтетике (50 профилей, 1e−9) и против независимой записи закона (1e−12); контрольная таблица ТЗ (16 клеток); Σc² = m; s = 1 возвращает тот же объект b; точный ноль сохраняется при s ≥ 1 и появляется при 0 < s < 1; равные b равны, одна мода c = 1, c ≥ 0; отказ при s = 0 / <0 / >4 / NaN. Пять движков на поле M1 (s = 0/0.5/1/2/4) 40 поколений бок о бок: частоты, веса, моменты пакетов, a и трекер равны; Σc² = m у каждого пакета; коэффициенты всех фаз preflight совпали при каждом s (1e−9); при s = 0 per-mode состояния ровно 0.
- `PathTests` (3) — Birth position s = 0 == Uniform побитово (12 переходов M1), Uniform одинаков при s = 1 и 4, s = 1 ≠ Uniform; WAV обеих записей каталога 2026-09-17 (E1, M1; сцены без ключа → default 1) == рендер текущего кода побитово по A/B/monitor (прямое сравнение, не реплей каталога — старые записи по правилу пользователя не подгоняются); смена s на неподвижном поле через 0 и 1 — без пакета, блоки и состояния равны двойнику; последовательность s по переходам E1 [4, 0, 0.5, 1, 2, 0]: каждый пакет со своим s, per-mode история не стирается при s = 0; снапшот v5 продолжает точно, v4-снимок без ключа импортируется как s = 1 (тот же звук), отказы (версия 4 с моделью v5, s вне диапазона / NaN в снимке).
- `SceneTests` (2) — сцена M2: клетки == M1, стороны отличаются только `birth_strength` (1 / 4), настройки M1, side_gain Б ≠ старый 0.93, диск == сборщик, Continue раннера с 6 с 200 блоков побитово (A/B/monitor), без клипа, display обеих сторон; `set_param` 4.5 отвергается, живая смена ручки через журнал (сразу и `at`), Continue после неё точен, `copy_side` переносит ручку, выход конечен; headless стенд: 17 строк панели, окно ≤ 880 px при низком столе, `FIGURE_HEAD_H` 62, подсказка и подпись помещаются, слайдер ручки при Birth position (drag даёт ≈2.0), текст `1.00  Birth position only` без слайдера при Uniform (клик не редактирует), четвёртая строка шапки нарисована в обоих состояниях.

## 4k. `tests/test_objects_decay.py` — Objects: затухание от истории клеток, D1–D3 (23)

ТЗ `memory/req-objects-decay-2026-09-18.md` (раздел 5), preflight `memory/research/objects-decay-preflight-2026-09-18.json`. Гейт 1q. Скретч: `artifacts/_od_tests/`.

- `RegistryTests` (2) — ручка `decay_law` Fixed/Common/Modal (0..2, default 0) сразу после `decay_s`; отсутствие ключа = Fixed; модель `n4_v6` / STATE_VERSION 6, v5 в `COMPATIBLE_STATES`; константы 0.08 / 0.5 / 0.020; правило сочетания (адаптивный закон только при Laplace + full) отвергается реестром, движком (параметры не меняются), сценой, конструктором движка; стенд блокирует обе стороны (текст с причиной у закона при Figure/8x8, Spectrum и full при адаптивном законе); `display` отдаёт закон и T-диапазоны фигур; все 15 сцен Objects грузятся с ключом. Раннер отвергает `set_param` / `copy_spectrum`, ведущие к недопустимому сочетанию, ещё при постановке (учитывая уже поставленные команды); отказ на применении не попадает в журнал.
- `HistoryTests` (3) — возраст клеток движка == независимая модель по блокам: glider через шов, смерть и повторное рождение (новая история), непрерывная жизнь, неизменное поле, ручка, лишний `update_field`, restore, удаление и возврат клетки до рендера (не событие), 6 и 2 gen/s; пауза КА D2 — история идёт; целевые T и частоты на сэмплах переходов preflight (D1 40, D2 22 + 4 границы пауз, D3 108 переходов со split/merge) для Common и Modal ≤ 1e−9.
- `MathTests` (3) — строки P суммируются в 1, P блинкера (0.5/0/0.5 и 1/6, 2/3, 1/6), инвариантность к знаку и повороту базиса кратной группы (Octagon II); перенос через шов и поворот 90° вместе с историей; равные возрасты → Common = Modal, одна мода; две одинаковые геометрии с разной историей — потери разные, частоты/веса/следующий пакет одинаковы; порядок q → T → gamma (Common = средняя СКОРОСТЬ), числа ТЗ на первой паузе D2, клип q в допуске и ошибка за ним.
- `KernelTests` (3) — адаптивное ядро == скалярный эталон (20-мс сглаживание, порядок обновления, r_j по моде) ≤ 1e−12 через смену целей; T60 свободной моды при замороженной gamma; при нулевом входе |z| не растёт при любых сменах потерь; огибающие мод НАСТОЯЩЕГО движка в обеих паузах D2: верх/низ в Modal растёт на десятки dB, в Common ровно, обе паузы одинаковы.
- `IsolationTests` (1) — D1/D2/D3 бок о бок: фигуры, слоты, клетки, частоты, веса, пакеты, a, Attack равны; различаются только потери; у адаптивного слота все gamma > 0.
- `StateTests` (6) — смена закона/Decay: без пакета, без сброса, без добавления энергии, непрерывно; погасший хвост не оживает; исчезнувший банк уносит applied/target gamma и держит цель; вернувшаяся фигура — новый банк, T = 0.08 с; хвост эпохи Fixed при переходе на адаптивный закон держит Decay момента переключения; возврат в Fixed сходится к глобальному r; WAV закреплённых записей E1/M1/M2 == текущий код побитово; снимок v5 → Fixed, u = 1 на живых клетках, тот же звук; отказы снимка; снимок v6 продолжает точно изнутри паузы D2, посреди перехода gamma (D1) и после split/merge (D3).
- `ScriptTests` (2) — ключ сцены `script` (паузы D2): ставится при Start, отпускании паузы остановленной сцены и Restart, снимается Stop, не дублируется при Start во время хода; неверные записи отвергаются; запись каталога со сценарием: команды в журнале с пометкой `script`, реплей match, Continue с конца не повторяет паузы.
- `ControlTests` (1) — D1: средняя целевая и применённая gamma Common за 5…20 с в пределах 1 % от ln(1000)/0.947926 (= `fixed_T60_s` preflight); Common T зрелого цикла ≈ 0.651/0.415/1.156/0.853/0.735 с.
- `SceneTests` (2) — сцены == preflight (клетки, темп, длительность), стороны отличаются только законом (D1 — и Decay), общие настройки ТЗ, side_gain, диск == сборщик; паузы D2 в журнале на точных сэмплах; окна калибровки A−B ≤ 1 dB, без клипа; headless стенд: 18 строк, окно ≤ 880 px, три кнопки закона внутри панели, строки шапки и текст неактивных ручек помещаются до колонки Patterns, клик по Common переключает, Spectrum под законом не редактируется, при Figure кнопки закона заблокированы.
