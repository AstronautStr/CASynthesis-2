...
Всё запускается одной командой `python check.py` (все гейты: юнит-тесты, стенд S1–S7, демо S/N, N0–N3, golden-master, UI-кадр). Ниже — каждый тест и
...
- `BenchTests` (1) — headless: 14 движков → 5 рядов, оверлей, слова режимов, `display` обеих сторон (`tuning` False/True), выбор Б, отрисовка панели N3.
